...
on using good open-source software and learning new workflows. This is a list of
free software packages that comprise an elegant, powerful, and integrated set of
tools for doing reproducible analyses and reports. These are my favorite tools
now, in part because they enable me to create dynamic documents. R is a
fantastic option for managing and analyzing data and creating figures. RStudio
Desktop is an excellent editor for working with R that also simplifies using
Quarto, Git, and GitHub too. That integration is powerful. Quarto is software
for generating dynamic documents that render to a variety of output formats
(HTML, PDF, Word, PowerPoint, etc.), while Git and GitHub together support
version control, collaboration, and sharing materials.
:::
...
contains data about references you cite in the report, while Quarto uses the CSL
file to format the citations and the reference list. I'm using similar file
names to stay organized: they show which script creates the report. Saving all
the files follows the preservation principle. One can rapidly reproduce the
