...
title-slide-attributes: 
  data-notes: 1 hr (45 min talk+ 15 min Q & A) My name is Steven Pierce. 
              Welcome to my session. My talk today focuses on some principles, 
              practices, and tools that I use to generate reproducible 
              statistical analyses and evaluation reports. My goal is encourage 
              evaluators to build their capacity and competency by adopting 
              practices and tools for reproducible research. Hopefully, sharing 
              my experience and insights will persuade you why that's 
              beneficial, motivate you to do so, and accelerate your learning 
              journey. My slides are in an HTML file available from the link at 
              the bottom of the screen. They have many links to resources built 
              right in and are themselves a reproducible product. 
...
Why should we aim for reproducibility?, and How do we achieve reproducibility?
Then I'm going to do show you a demonstration. Let's get started with the first
question.
:::
...
the you from 6 months ago who assumed you'd just remember something important 
about the project that you've now forgotten and so skipped documenting it."

*Time elapsed should be ~24 min by this point based on rehearsal.*
:::
...
how scripts link to output files. You can also use data flow diagrams to help 
show people the relationships between files. Finally, consider creating a 
primary rendering script that automates running all your other scripts in the 
correct order. 
:::
...
::: {.notes}
Here's an example data flow diagram that illustrates several of the tips I just 
shared. Boxes represent files and arrows show which files are inputs to other 
files or outputs created by them. Scripts and their outputs have the same 
filename except for the extension that denotes file type. Notice how the Render 
script actually runs three other scripts, one for data cleaning, plus one that 
creates some slides, and another that creates a report. This diagram makes it 
easy to see how 10 files are related. 
:::

## Automation Tips {background-image="graphics/robot-6654025_1280.png" background-size="45%" background-position="right" background-repeat="no-repeat"}
...
::: {.notes}
With respect to automation, I suggest using scripts to do as much of the work as 
possible. Manual steps reduce the efficiency and reproducibility of your work. 
They are more error-prone than well-written code. Each script should start from 
a clean R session--an empty workspace--and use code to load the data it needs 
to run. You also want to render each script in its entirety frequently as you 
develop it. That's the best way make sure the whole thing works correctly. 

Leverage the fact that computers are inhumanely patient. They're great at
tedious, repetitive tasks that bore or fatigue humans into making errors. If
something is mind-numbingly tedious, write code to get the computer to do it for
you. However, computers are also literal-minded idiots. They do exactly what you
tell them to do, whether or not that makes sense or is what you intended. Put
your time into giving them the right instructions and checking that you got the
right results.
:::
...
compendium must either include the raw data and relevant metadata, or tell 
people how to get them from some accessible source. Documenting methodology 
decisions and rationales is important too, as are the data management and 
analysis scripts that actually produced your output. Keeping version histories 
of repository contents is a major bonus, but not strictly required. I recommend 
that your scripts end with code that automates recording information about the 
software environment and versions you used to meet the documentation criterion 
for reproducibility.
:::
...
* Integrate narrative text with code 
* Add functions & help files to your compendium
* Include citations & reference lists 
  
::::

::: {.notes}
Here are some quick tips on applying the3 integration priciple. Attaching 
metadata such as variable and value labels directly to your data is an obvious 
way to make that metadata available to users of your compendium. THe nature of 
Quarto scripts allows you to integrate narrative text with the code used to 
produce results. If you need custom R functions, add them to your compendium 
(which is also an R package) and include help files that explain what they do 
and how to use them. Finally, take advantage of Quarto's support for building 
reference lists and cite sources throughout your scripts. 
:::
...
::: {.notes}
This table briefly summarizes tips on applying the separation principle. I've 
mentioned the first three rows already, so here I want to highlight the last 
three. In larger projects, sometimes it is useful to split processes into 
multiple scripts. An obvious split is between data management and data analysis 
scripts. Another thing I like to do is separate my draft outputs from the 
versions of final deliverables sent to clients or stakeholders by date-stamping 
the filenames for final deliverables. Finally, Git automatically separates the 
current versions of files from their version histories. 
:::
...
sentence. Code chunks allow you to do more substantial computing, like reading 
in or managing data, fitting models, or creating and then inserting more complex 
pieces of output like tables and figures. 
:::
